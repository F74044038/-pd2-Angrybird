...
{
    r=8;                                                        //// to inital the value no real
    ...
    QPixmap background;
     ...
     background=background.scaled(754*1.5,628*1.5);
    ...
    /////////////////////////////////////////////// my arrow
     ...
     arrow.load(":/arrow.png");
      ...
      scene->addItem(gamearrow);
    //////////////////////////////////////////  create basic world bird ,obstacle,pig
    basicgame();
...
bool MainWindow::eventFilter(QObject *, QEvent *event)
{
            ...
            if(  (x>=1.0&&x<=2.5) && (y>=15.6&&y<=16.8)   )
            {
             ...
             if(allbird[whichbird]->used==true&&m->button()==Qt::RightButton&&whichbird<3){                                                                                                       ///// if bird was used and x-axis not change
                   ...
                   if(a.x<0.1&&a.y<0.1){
                        ...
                        if(whichbird==3){
                            allbird[3]=new greenbird(6.7f,8.0f,1,&timer,QPixmap(":/greenbird.png").scaled(width()/32*2,height()/18*2),world,scene);               ///green bird
                        }
                    }
                    return true;
             }
    ...
    if(event->type() == QEvent::MouseMove&&ongame==true)
    {
       ...
       str = "MouseMove" + QString::number(x) +", " + QString::number(   y    );
    ...
    //////////////////////////////////////////////////////////   release
    if(event->type() == QEvent::MouseButtonRelease&&ongame==true)
    {
        // TODO : add your code here
...
void MainWindow::basicgame()                /// to create basic item
{
    // Create bird (You can edit here)
    whichbird=0;                                        //// to choose use which bird
    win=false;
    allbird[0] = new Bird(6.7f,8.0f,1,&timer,QPixmap(":/bird.png").scaled(width()/32*2,height()/18*2),world,scene);                                      ///red bird
    //////////////////////// create pig
    allpig[0]=new pig(23.0f,10.0f,1,&timer,QPixmap(":/pig.png").scaled(width()/32*2,height()/18*2),world,scene);
    ...
    ///////////////////////  create obstacle
    barrier[0]=new Obstacle(19.0f,5.5f,1.5,5,&timer,QPixmap(":/26_158.png").scaled(width()/32*1.5,height()/18*5),world,scene);
    barrier[1]=new Obstacle(27.0f,5.5f,1.5,5,&timer,QPixmap(":/26_158.png").scaled(width()/32*1.5,height()/18*5),world,scene);
    barrier[2]=new Obstacle(23.0f,8.8f,9.5,1,&timer,QPixmap(":/304_31.png").scaled(width()/32*9.5,height()/18*1),world,scene);
     ...
     barrier[5]=new Obstacle(23.0f,13.0f,7,1,&timer,QPixmap(":/304_31.png").scaled(width()/32*7,height()/18*1),world,scene);
     ongame=true;
    ////////////////////////
}
void MainWindow::freebasicgame()        ////to delete basic item
{
